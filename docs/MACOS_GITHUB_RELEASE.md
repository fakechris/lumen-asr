...
```text
ad-hoc 签名 → Apple Silicon DMG → SHA-256 校验 → GitHub Release
```
...
推送形如 `vMAJOR.MINOR.PATCH` 的 tag 后，[release-desktop.yml](../.github/workflows/release-desktop.yml) 会：

1. 从 tag 解析版本，并把它注入 Tauri App bundle；
2. 在 Apple Silicon runner 上构建 `aarch64-apple-darwin`；
3. 在 Windows runner 上构建 x64 NSIS 安装包；
4. 挂载最终 DMG，检查其中 `.app` 的版本、主程序、CPU 架构、签名完整性和 `Signature=adhoc`；
5. 生成命名稳定的 macOS arm64 DMG、Windows x64 安装包和 `SHA256SUMS.txt`；
6. 只有两个受支持平台都构建成功后，才创建 GitHub Release 并上传全部文件。

最终 Release 包含：

```text
Lumen-ASR-v0.1.0-arm64.dmg
Lumen-ASR-v0.1.0-windows-x64-setup.exe
SHA256SUMS.txt
```

正式发布范围仅包括 Apple Silicon Mac 和 Windows x64；不构建 Intel Mac
或 Windows x86 安装包。
...
随后在 GitHub 的 Actions 页面查看 `Release desktop installers`。成功后，Release 会自动出现在仓库的 Releases 页面。
...
将 DMG 和 `SHA256SUMS.txt` 放在同一目录，并校验 Apple Silicon 条目：
...
1. Apple Silicon 用户下载 `arm64.dmg`；

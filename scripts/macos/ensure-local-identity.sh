...
#   2. "Lumen Local Codesign" (self-signed, created here or via Keychain Access)
#   3. Valid "Apple Development: …" (free Personal Team — renew yearly in Xcode)
#   4. ad-hoc `-` (always works; TCC Mic/AX often break every rebuild)
#
# Why not only ad-hoc?
#   Ad-hoc has no certificate. Every re-sign → new cdhash → macOS TCC treats the
#   app as a different binary → Accessibility / Microphone must be re-granted.
#

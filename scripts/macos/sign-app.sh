#!/usr/bin/env bash
# Sign a .app with the best stable identity available.
# Usage: sign-app.sh [/path/to/Lumen ASR.app]
...
if [[ "$IDENTITY" == "-" ]]; then
  ...
  echo "  WARN: ad-hoc changes cdhash every rebuild → Mic/Accessibility often need re-grant."
...
codesign -dv --verbose=2 "$APP" 2>&1 | rg -i "Authority|Signature|TeamIdentifier|Identifier|Format|flags=" || true

if [[ "$IDENTITY" != "-" ]]; then
  REQ="$(codesign -d -r- "$APP" 2>&1 | rg 'designated =>' || true)"
